...
class Window:
	...
	def __init__(self, root):
		...
		Button(self.root, text = "Save", command = self.savefile).grid(row=1, column = 0)
		Button(self.root, text = "Open", command = self.openfile).grid(row=2, column = 0)
		Button(self.root, text = "Exit", command = exit()).grid(row=3,column=0)
